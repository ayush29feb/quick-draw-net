  ...
  def inference(self, images, training=True):
    ...
    with tf.name_scope('L1') as scope:
      weights1 = tf.Variable(tf.truncated_normal(
          (15, 15, 1, 64), stddev=self._init_stddev), name='weights')
    ...
    with tf.name_scope('L4') as scope:
      ...
      conv4 = tf.nn.conv2d(relu3, weights4, [1, 1, 1, 1], padding='SAME', name='conv')
    ...
    with tf.name_scope('L5') as scope:
      ...
      conv5 = tf.nn.conv2d(relu4, weights5, [1, 1, 1, 1], padding='SAME', name='conv')
    ...
    with tf.name_scope('L6') as scope:
      weights6 = tf.Variable(tf.truncated_normal(
          (7, 7, 256, 512), stddev=self._init_stddev), name='weights')
      biases6 = tf.Variable(tf.constant(self._init_stddev, shape=(512,)), name='biases')
      
      fc6 = tf.nn.conv2d(pool5, weights6, [1, 1, 1, 1], padding='VALID', name='fc')
      relu6 = tf.nn.relu(tf.nn.bias_add(fc6, biases6), name='relu')
    ...
    with tf.name_scope('L7') as scope:
      ...
      fc7 = tf.nn.conv2d(dropout6, weights7, [1, 1, 1, 1], padding='VALID', name='fc')
      relu7 = tf.nn.relu(tf.nn.bias_add(fc7, biases7), name='relu')
    ...
    with tf.name_scope('L8') as scope:
      weights8 = tf.Variable(tf.truncated_normal(
          (1, 1, 512, 2), stddev=self._init_stddev), name='weights')
      biases8 = tf.Variable(tf.constant(self._init_stddev, shape=(2,)), name='biases')
      
      fc8 = tf.nn.conv2d(dropout7, weights8, [1, 1, 1, 1], padding='VALID', name='conv')
      fc8b = tf.nn.bias_add(fc8, biases8)
    
    logits = tf.reshape(fc8b, [-1, 2])
    return logits
